...
class FormSection extends Component {
    ...
    render() {
        ...
        const fields = tag === undefined
            ? schema
            : getTaggedFields(schema, tag);
                ...
                {fields &&
                    fields.map(fieldSchema => {
                        ...
                        const fieldComponent = fieldSchema._meta.component ||
                            "text";
                        const fieldName = fieldSchema._meta.name ||
                            camelize(fieldSchema._flags.label);
                        ...
                        const options = {
                            ...fieldSchema._meta,
                            required: fieldSchema._flags.presence ===
                                "required",
                            name: fieldName,
                            label: fieldSchema._flags.label,
                            key: fieldName,
                            allowed: optionValues,
                            default: fieldSchema._flags
                        ...
                        switch (fieldComponent) {
                            case "text":
                                options.placeholder = fieldSchema._examples[
                                    0
                                ] || undefined;
                                break;
                            ...
                            case "form":
                                ...
                                const schemaProvider = options.type === "object"
                                    ...
                                    : fieldSchema._inner.items[
                                          0
                                      ]._inner.children; // we get an array of [key, schema] from the first valid item type of the array - that should be an object

                                options.schema = schemaProvider.map(
                                    c => c.schema
                                );
                                break;
                        }

                        const fieldComponentCreator = context[
                        ...
                        const fieldEvents = {
                            ...
                            onAutocompleteSearch: __onAutocompleteSearch(
                                onAutocompleteSearch
                            ),
